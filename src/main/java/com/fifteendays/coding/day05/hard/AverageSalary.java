package com.fifteendays.coding.day05.hard;

import com.fifteendays.coding.day05.Employee;
...
public class AverageSalary {
    ...
    public static void main(String[] args) {
        AverageSalary instance = new AverageSalary();
        instance.init();

        //Q1. Given List<Employee>, find average salary of employees whose name starts with 'A'.

        Double averageSalary = employees.stream().filter(employee -> employee.getName().startsWith("A"))
                .collect(Collectors.averagingDouble(Employee::getSalary
                ));

        System.out.println(averageSalary);

        //Q2. Find the employee with maximum salary in each department (grouping + max).
        Map<String, Optional<Employee>> employeeMapByMaximumSalary =
                employees.stream()
                        .collect(Collectors.groupingBy(Employee::getDepartment,
                                Collectors.maxBy(Comparator.comparingDouble(Employee::getSalary))));
        System.out.println(employeeMapByMaximumSalary);
